<?php 
	session_start();/*хранения информации;запускаться вручную*/
	if(!isset($_SESSION['uid']) || $_SESSION['status']!=0){
		header("Location:/agai/aga.php");
	}
...
<?php while($item = $res->fetch_assoc()){?>
	<tr><td><?=$item['id']?></td><td><img src="img/<?=$item['img']?>" style="width:100px;"></td><td><?=$item['title']?></td><td><?=$item['text']?></td><td><a href="deleteNews.php?id=<?=$item['id']?>"><button class="btn btn-danger">Delete</button></a></td></tr>
<?php } ?>
</tbody>
</table>
</body>
